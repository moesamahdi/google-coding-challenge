...
def sort_lex(to_be_sorted , base_list):
    return [x for _, x in sorted(zip(base_list, to_be_sorted))]
...
class VideoLibrary:
    ...
    def get_all_videos(self):
        ...
        vid_list = list(self._videos.values())
        title_list = []

        for vid in vid_list:
            title_list.append(vid.title)
        sorted_vid_list = sort_lex( vid_list , title_list )
        
        return sorted_vid_list
